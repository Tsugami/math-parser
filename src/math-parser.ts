...
const isNumber = (v: any): v is number => NUMBERS.test(v)
...
const isTokenOperator = (token: Token, operator: OperatorTypes): token is OperatorToken => token.type === 'operator' && token.value === operator
...
export const parser = (tokens: Token[]): number => {
    ...
    const parseNumber = (): number => {
        const matchNum = () => {
            ...
            return tokens.slice(startPos, current).map(token => token.value).join('');
        }
